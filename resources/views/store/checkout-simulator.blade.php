...
@section('content')
    <div class="container-xl mt-3">
            ...
            <div class="card-body">
                <form class="row g-2 align-items-end" method="get" action="{{ url('/loja/checkout/simulador') }}">
                    ...
                    <div class="col-12 col-md-2">
                        <label class="form-label mb-1">País (ISO2)</label>
        ...
        <div class="row g-3">
                ...
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        ...
                        @if (count($quote['carriers'] ?? []) === 0)
                            <div class="alert alert-secondary mb-0">Sem transportadoras disponíveis.</div>
                        @else
                                ...
                                @foreach (($quote['carriers'] ?? []) as $c)
                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <div class="small text-muted">{{ $c['delay'] ?? 'Prazo não definido' }}</div>
                                        </div>
                                        <div class="text-end">
                                            <div class="fw-semibold">{{ number_format((float) ($c['price_ex_vat'] ?? 0), 2, ',', ' ') }} €</div>
            ...
            <div class="col-12 col-lg-6">
                <div class="card h-100">
                    <div class="card-header fw-semibold">Métodos de pagamento</div>
                    <div class="card-body">
                        @if (count($quote['payment_methods'] ?? []) === 0)
                            <div class="alert alert-secondary mb-0">Sem métodos disponíveis para as transportadoras selecionadas.</div>
                        @else
                            <div class="list-group">
                                @foreach (($quote['payment_methods'] ?? []) as $m)
                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="text-end">
                                            <div class="fw-semibold">{{ number_format((float) ($m['fee_ex_vat'] ?? 0), 2, ',', ' ') }} €</div>
