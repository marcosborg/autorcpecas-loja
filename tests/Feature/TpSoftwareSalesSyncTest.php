...
test('falls back to GET when TP sales endpoint rejects POST with 404', function () {
    config()->set('storefront.catalog_provider', 'tpsoftware');
    config()->set('tpsoftware.sales_sync.enabled', true);
    config()->set('tpsoftware.sales_sync.method', 'auto');
    config()->set('tpsoftware.base_url', 'https://api.tp.software/api/v1');
    config()->set('tpsoftware.token', 'tp-token-test');
    config()->set('tpsoftware.token_param', 'tokens');
    config()->set('tpsoftware.use_auth_header', false);
    ...
    Http::fake([
        'https://api.tp.software/api/v1/ecommerce-generate-sales-order*' => function (Request $request) {
            if ($request->method() === 'POST') {
                return Http::response('Cannot POST /api/v1/ecommerce-generate-sales-order', 404);
            }
    ...
    $user = User::factory()->create([
        'email' => 'tp-sync-get@example.com',
    ]);

    $order = Order::query()->create([
        'user_id' => $user->id,
        'order_number' => 'ORC-TPSYNC-0002',
        'status' => 'awaiting_payment',
        'currency' => 'EUR',
        'vat_rate' => 23,
        'subtotal_ex_vat' => 40,
    ...
    $order->refresh();
    expect((string) data_get($order->payment_method_snapshot, 'tpsoftware_sale_sync.status'))->toBe('success')
        ->and((int) data_get($order->payment_method_snapshot, 'tpsoftware_sale_sync.tp_sale_id'))->toBe(12345);
});
